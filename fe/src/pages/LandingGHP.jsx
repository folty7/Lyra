...
GEMINI_API_KEY=•••••`}
            ...
            {/* Legal sections */}
            <section id="privacy" className="relative z-10 w-full max-w-4xl mx-auto px-6 sm:px-10 pt-16 pb-4">
                <details className="rounded-2xl bg-gradient-to-b from-white/[0.04] to-white/[0.01] border border-white/[0.07] group">
                    <summary className="flex items-center justify-between px-6 py-4 cursor-pointer list-none text-white/60 text-[13px] font-medium select-none hover:text-white transition-colors">
                    ...
                    <div className="px-6 pb-8 pt-2 space-y-4 border-t border-white/[0.05]">
                        ...
                        {[
                            { t: "Data we collect", b: "Lyra accesses your display name, profile picture, 100 most recently saved tracks (title, artist, album, duration, popularity, release year), and your top tracks and artists for dashboard statistics. We do not collect your email address, payment information, or listening history beyond the above." },
                            ...
                            { t: "Contact", b: "Questions? Email ondrej4a@gmail.com." },
                        ].map((item, i) => (
                            <div key={i}>
                                <h3 className="text-white/80 text-[13px] font-medium mb-1">{item.t}</h3>
                                <p className="text-white/50 text-[13px] leading-relaxed">{item.b}</p>
                            </div>
                        ))}
                    </div>
                </details>
            ...
            <section id="terms" className="relative z-10 w-full max-w-4xl mx-auto px-6 sm:px-10 pt-3 pb-12">
                <details className="rounded-2xl bg-gradient-to-b from-white/[0.04] to-white/[0.01] border border-white/[0.07] group">
                    <summary className="flex items-center justify-between px-6 py-4 cursor-pointer list-none text-white/60 text-[13px] font-medium select-none hover:text-white transition-colors">
                    ...
                    <div className="px-6 pb-8 pt-2 space-y-4 border-t border-white/[0.05]">
                        <p className="text-white/40 text-[11px] pt-4">Last updated: April 27, 2026</p>
                        {[
                            { t: "Eligibility", b: "You must be at least 13 years old and hold a valid Spotify account to use Lyra." },
                            { t: "Spotify authorisation", b: "Lyra accesses your Spotify account only with your explicit consent via Spotify OAuth 2.0. You may revoke access at any time via your Spotify account settings. Lyra requests only the minimum required scopes: user-library-read, user-top-read, and playlist-modify-private." },
                            { t: "Acceptable use", b: "Do not use Lyra for any unlawful purpose or in violation of Spotify's or Google's terms of service. Do not attempt to reverse-engineer, scrape, or abuse the Service or its underlying APIs." },
                            { t: "AI-generated content", b: "Playlist suggestions are generated by Google Gemini AI and provided for convenience only. We make no guarantee about their accuracy or appropriateness. You are responsible for reviewing suggestions before pushing them to Spotify." },
                            { t: "Disclaimer of warranties", b: "The Service is provided \"as is\" without warranties of any kind. Use of the Service is at your own risk." },
                            { t: "Limitation of liability", b: "To the fullest extent permitted by law, Andrej Folta shall not be liable for any indirect, incidental, or consequential damages arising from your use of or inability to use the Service." },
                            { t: "Governing law", b: "These Terms are governed by the laws of the Slovak Republic." },
                            { t: "Contact", b: "Questions? Email ondrej4a@gmail.com." },
                        ].map((item, i) => (
                            <div key={i}>
                                <h3 className="text-white/80 text-[13px] font-medium mb-1">{item.t}</h3>
